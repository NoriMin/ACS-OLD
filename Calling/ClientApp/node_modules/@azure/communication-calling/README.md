...
The `createCallAgent` method takes a `CommunicationUserCredential` as an argument, which accepts a [user access token](https://docs.microsoft.com/azure/communication-services/quickstarts/access-tokens).
...
To place a call to another Communication Services user, invoke the `call` method on `callAgent` and pass the CommunicationUser that you've [created with the Communication Services Administration library](https://docs.microsoft.com/azure/communication-services/quickstarts/access-tokens).
