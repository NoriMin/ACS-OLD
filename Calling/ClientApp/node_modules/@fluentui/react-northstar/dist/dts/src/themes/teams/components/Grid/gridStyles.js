"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var tslib_1 = require("tslib");
var getCSSTemplateValue = function (template, gap) {
    if (gap === void 0) { gap = ''; }
    return Array.from({ length: template }, function () { return '1fr'; }).join(" " + gap + " ");
};
exports.gridStyles = {
    root: function (_a) {
        var props = _a.props, _b = _a.variables, height = _b.height, width = _b.width, defaultColumnCount = _b.defaultColumnCount, gridGap = _b.gridGap, padding = _b.padding;
        var rows = props.rows, _c = props.columns, columns = _c === void 0 ? !props.rows && defaultColumnCount : _c;
        return tslib_1.__assign(tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({ height: height,
            width: width,
            padding: padding,
            gridGap: gridGap, display: ['grid', '-ms-grid'], justifyContent: 'space-evenly' }, (rows && !columns && { gridAutoFlow: 'column' })), (rows && {
            gridTemplateRows: getCSSTemplateValue(rows),
            msGridRows: getCSSTemplateValue(rows, gridGap),
        })), (columns && {
            gridTemplateColumns: getCSSTemplateValue(columns),
            msGridColumns: getCSSTemplateValue(columns, gridGap),
        })), { '& > *': { outlineOffset: '-3px' } });
    },
};
